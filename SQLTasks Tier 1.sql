...
SELECT DISTINCT CONCAT_WS(m.firstname, ' ', m.surname) as membername, f.name as tenniscourt
...
SELECT b.bookid, f.name, concat(m.firstname, ' ', m.surname) AS membername,
...
SELECT sub.bookid, sub.name, concat(m.firstname, ' ', m.surname) as membername, sub.cost
FROM (SELECT
      f.name, 
      b.memid, 
      b.bookid,
...
SELECT m.memid, m.surname, m.first,
        CASE WHEN m.recommendedby = 0 THEN null
        ELSE r.membername END AS recommender 
FROM Members AS m
LEFT JOIN (
    SELECT memid, (surname ||','|| firstname) as membername
    FROM Members) AS r
    ON m.recommendedby = r.memid
ORDER BY m.surname, m.firstname

/* Q12: Find the facilities with their usage by member, but not guests */
SELECT f.name AS facility, COUNT(b.bookid) AS n_bookings,  b.memid, m.firstname, m.surname
FROM Bookings AS b
LEFT JOIN Members AS m
    ON b.memid = m.memid
LEFT JOIN Facilities as f
